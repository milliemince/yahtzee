...
state1 = State([2, 3, 4, 5, 6], 0, Moves(false, false, false, false, false, false, false, false, false, false, false, false, false), 104)
...
function best_fit_line(list_of_tuples)
    """ returns slope and y-intercept of best fit line of scatterplot with move (pay off, probability) data points """
    x_coord = []
    ...
    x_bar = (mysum(x_coord))/(length(x_coord))
    y_bar = (mysum(y_coord))/(length(y_coord))
    m_x = 0
    for i in (1:(length(x_coord)))
        m_x = m_x + (x_coord[i] - x_bar) * (y_coord[i] - y_bar)
    end
    m_y = 0
    for i in (1:(length(x_coord)))
        m_y = m_y + ((x_coord[i] - x_bar)^2)
    end
    m = (m_x)/(m_y)
    b = y_bar - m * (x_bar)
    return (m, b)
...
function simpl_4OAK(state, die)
  """ returns probability of reaching a 4 OAK given state and specific dice number. if 4 OAK of specified number
      is already satisfied, probability is 1.0 """
  c = count_d(state.dice)
  count = c[die]
  if geq(count, 4)
    ...
    prob = reaching_x(state, die, 4)
  end
  return prob
end

function upper_section_4OAK_probabilities(state)
  """ is this function necessary? returns dictionary of probabilites of reaching 4 of a kinds for upper section """
  bools = state.remaining_moves
  Prob_Dict = Dict()
  if bools.one == false
    Prob_Dict[one] = simpl_4OAK(state, 1)
  end
  if bools.twos == false
    Prob_Dict[twos] = simpl_4OAK(state, 2)
  end
  if bools.threes == false
    Prob_Dict[threes] = simpl_4OAK(state, 3)
  end
  if bools.fours == false
    Prob_Dict[fours] = simpl_4OAK(state, 4)
  end
  if bools.fives == false
    Prob_Dict[fives] = simpl_4OAK(state, 5)
  end
  if bools.sixes == false
    Prob_Dict[sixes] = simpl_4OAK(state, 6)
  end
  return Prob_Dict
end
...
iboard = ["Scoreboard" "You" "Computer"; "ones (sum of 1s)" "-" "-"; "twos (sum of 2s)" "-" "-"; "threes (sum of 3s)" "-" "-"; "fours (sum of 4s)" "-" "-";
      ...
      "Additional Yahtzees (100 pts. each)" "-" "-"; "Chance" "-" "-"; "Total Score" 0 0]
...
function remove!(a, item)
  """ modifies input list by removing specified element """
...
function filter_moves(combos, move)
  """ Do we need this??? """
    if move == "ones"
      remove!(combos, "1x1")
      remove!(combos, "1x2")
    ...
    elseif move == "4 OAK"
      remove!(combos, "4 OAK")
    elseif move == "Full House"
      remove!(combos, "Full House")
    elseif move == "Sm. Straight"
      remove!(combos, "Sm. Straight")
    elseif move == "Lrg. Straight"
      remove!(combos, "Lrg. Straight")
    elseif move == "Yahtzee"
      remove!(combos, "Yahtzee")
    elseif move == "Chance"
      remove!(combos, "Chance")
    end
end
